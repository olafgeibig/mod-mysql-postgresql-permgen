...
docsApp.directive.sourceEdit = function(getEmbeddedTemplate) {
  ...
  function read(text) {
    ...
    angular.forEach(text ? text.split(' ') : [], function(refId) {
      // refId is main.html-343, so we need to strip the unique ID when exporting the name
...
docsApp.serviceFactory.openPlunkr = function(templateMerge, formPostData, prepareEditorAssetTags, prepareDefaultAppModule) {
  return function(content) {
    ...
    angular.forEach(allFiles, function(file, index) {
      if (file.content && file.name != 'main.html') {
        postData['files[' + file.name + ']'] = file.content;
      }
    });

    postData['files[main.html]'] = templateMerge(indexHtmlContent, indexProp);
